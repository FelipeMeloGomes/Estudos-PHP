...
class AdminUsuarios extends AdminControlador
{
        ...
        if (isset($dados)) {
            ...
            if ($this->validarDados($dados)) {
                ...
                if (empty($dados['senha'])) {
                    $this->mensagem->alert('Informe uma senha para o usuário')->flash();
                } else {
                    $usuario = new UsuarioModelo();
    ...
    public function editar(int $id): void
    {
        ...
        if (isset($dados)) {
            if ($this->validarDados($dados)) {
                ...
                if ($usuario->salvar()) {
                    $this->mensagem->success('Usuário atualizado com sucesso')->flash();
                    Helpers::redirecionar('admin/usuarios/listar');
                } else {
                    $usuario->mensagem()->flash();
                }
    ...
    {
        if (empty($dados['nome'])) {
            $this->mensagem->alert('Informe o nome do usuário')->flash();
            return false;
        }
        if (empty($dados['email'])) {
            $this->mensagem->alert('Informe o e-mail do usuário')->flash();
            return false;
        }
        if (!Helpers::validarEmail($dados['email'])) {
            $this->mensagem->alert('Informe um e-mail válido!')->flash();
            return false;
        }
        if (!empty($dados['senha'])) {
            if (!Helpers::validarSenha($dados['senha'])) {
                $this->mensagem->alert('A senha deve ter entre 6 e 50 caracteres')->flash();
                return false;
            }
        }
    ...
    public function deletar(int $id): void
    {
        if (is_int($id)) {
            $usuario = (new UsuarioModelo())->buscaPorId($id);
            if (!$usuario) {
                $this->mensagem->alert('O usuário que você está tentando deletar não existe!')->flash();
                Helpers::redirecionar('admin/usuarios/listar');
            } else {
                if ($usuario->deletar()) {
                    $this->mensagem->success('Usuário deletado com sucesso!')->flash();
                    Helpers::redirecionar('admin/usuarios/listar');
                } else {
                    $this->mensagem->error($usuario->erro())->flash();
                    Helpers::redirecionar('admin/usuarios/listar');
                }
            }
        }
